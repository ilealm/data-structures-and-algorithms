...
Your function should use slice within a loop and return an array of each successive string result from l
osing letters to the sharpener, until nothing is left.
...
const listFoods = (recipe) => {
  let arrResult = [];
  let positionSpc = 0;
  let arrIngredientes = recipe.ingredients
  arrIngredientes.forEach(ingredient => {
    positionSpc = ingredient.indexOf(' ', ingredient.indexOf(' ') + 1);
    arrResult.push(ingredient.slice(positionSpc+1));
  })
  return arrResult;
};
...
const stepActions = (recipe) => {
  let arrResult = [];
  let positionSpc = 0;
  let arrSteps = recipe.steps
  arrSteps.forEach(ingredient => {
    console.log(ingredient);
    positionSpc = ingredient.indexOf(' ', 1) // 1 means the first time ' ' appered
    arrResult.push(ingredient.slice(0, positionSpc));
  })
  return arrResult;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if (numberOfCharacters > str.length) {
    ...
    let newStr = str.substring(0,(str.length - numberOfCharacters));
    return newStr;
  }
};
...
Write a function named totalSumCSV that, given a string of comma-separated values (CSV) as input. (e.g. "1,2,3"), 
...
Write a function named extractVowels that takes in a string and returns an array where the first element 
is the original string with all the vowels removed, and the second element is a string of all the vowels that were removed, in alphabetical order.
...
const extractVowels = (str) => {
  let arrResult = [];
  // PART 1
  // returns an array where the first element is the original string with all the vowels removed
  let regex=/[aeiouAEIOU]/g;
  arrResult.push(str.replace(regex,'')); // remove all consonants

  // PART 2
  //second element is a string of all the vowels that were removed
  regex=/[^aeiouAEIOU]/g;
  let vowelStr = str.replace(regex,''); //get all the consonats
  // console.log('vowelStr ' + vowelStr);

  // turn consonants to array so I can sort them
  let tempArray = vowelStr.split('');
  // sort the consonats array
  tempArray.sort();

  // turn array to string
  let tempStr = tempArray.join('');
  // push the temp ordered string
  arrResult.push(tempStr);
