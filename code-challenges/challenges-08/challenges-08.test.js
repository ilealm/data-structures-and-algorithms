...
const oddValues = (arr) => {
  let arrReturn = arr.filter(value => (value % 2));
  return arrReturn;
};
...
const filterStringsWithVowels = (arr) => {
  let arrReturn = arr.filter(value => {
    let regex=/[aeiuoAEIOU]/g;
    return regex.test(value);
  });
  return arrReturn //regex.test('dfff') //arrReturn;
}
...
const notInFirstArray = (forbiddenValues, arr) => {
  let arrReturn = arr.filter(value => !(forbiddenValues.includes(value)));
  return arrReturn
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let arrReturn = arr.filter(value => ((value.baseStat) > minBaseStat) );
  return arrReturn;
};
...
const getStatName = (arr, minBaseStat) => {
  let arrReturn = []
  let arrStats = getBaseStatGreaterThan(arr, minBaseStat);

  arrStats.forEach(statEle =>{
    arrReturn.push(statEle.stat.name);
  });
  return arrReturn;
};
...
const getCharactersWithoutChildren = (arr) => {
  let arrReturn = arr.filter(family =>{
    return ( family.children == undefined )
  });
  return arrReturn;
};
...
const evenOddNumericValues = (arr) => {
  ...
  // //     if (!isNaN(value)) {
  // //      arrJustNums.push(value);
  // //   }
  // // });
  // // let arrReturn = arrJustNums.map()

  // console.log(arrJustNums)
};
