...
const getHouses = (arr) => {
  ...
  arr.forEach(elementHouse =>{
    houses.push(elementHouse.house);
  })
...
const hasChildrenValues = (arr, character) => {
  let varReturn = false;
  Object.values(arr).forEach(houses => {
    if ((houses.name === character) && (houses.children.length >0)){
      varReturn = true;
      console.log('in house of ' + houses.name + ' , children: ' + houses.children.length + ', have children:' + varReturn);
    }
  });
  return (varReturn);
  //this part works, but need to be refactored
  // let numChildren;
  // let varReturn = false;
  // arr.forEach(house => {
  //   numChildren = house.children.length;
  //   if ((house.name === character) && (numChildren>0)){
  //     varReturn = true;
  //     console.log('in house of ' + house.name + ' , children: ' + numChildren + ', have children:' + varReturn);
  //   }
  // })
  // return (varReturn);
};
...
const hasChildrenEntries = (arr, character) => {
  let varReturn = false;
  Object.entries(arr).forEach(houses => {
  // houses[0] contains the position of the array:. I need houses[1] where is the each object
    if ((houses[1].name === character) && (houses[1].children.length >0)){
      varReturn = true;
      // console.log('in house of ' + houses[1].name + ' , children: ' + houses[1].children.length + ', have children:' + varReturn);
    }
  });
  return (varReturn);
};
...
------------------------------------------------------------------------------------------------ */
//TA: this is working, not sure why is failing the test
const totalCharacters = (arr) => {
  let arrayWord = '';
  if (arr.length > 0){
    arr.forEach(word =>{
      arrayWord = arrayWord + word;
    })
    return(arrayWord.length); 
    //  return(arrayWord.length.toString()); 
  }
  else
    // return '0';
    return 0;
