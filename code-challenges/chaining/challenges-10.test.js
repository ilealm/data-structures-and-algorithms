...
const count = (target, input) => {
  let targetOcurrencies = 0
  input.forEach(value =>{
    value.forEach(subvalue =>{
      if (target === subvalue)
        targetOcurrencies++; })
  })
  return targetOcurrencies;
};
...
let findShortest = (data) => {
  ...
  let numHeight=0;
  data.forEach(character =>{
    let numHeight = new Number(character.height);
    if (numHeight < shortestHeight){
      shortestName = character.name;
      shortestHeight = numHeight;
    }
  })
  return shortestName;
};
