...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i=0; i<arr.length; i++){
    let powerNum = 1;
    for (let j=0; j<arr[i]; j++){ powerNum = 2 * powerNum; }
    returnArray.push(powerNum);
  }
  return returnArray;
};
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach(number =>{
    let powerNum = 1;
    for (let j=0; j<number; j++){ powerNum = 2 * powerNum; }
    returnArray.push(powerNum);
  })
  return returnArray;
};
...
const extractAbilities = (arr) => {
  let arrAbilities = arr.map(abilities => {
...
const extractStats = (arr) => {
  // let statName, statTotal;
  // let arrReturn = arr.stats.map(stat => {
  //   // return (stat.effort + stat.baseStat);
  //   statName = stat.stat.name;
  //   // need to pass this as an pair key/value
  // })
  // return arrReturn;
};
