...
const findAnything = (arr, target) => {
  let arrReturn = arr.filter(word =>{ if (word.includes(target)) return word;})
  return arrReturn;
};
...
const findEvery = (arr, target) => {
  let inAllWord = true;
  arr.forEach(word => {if (!(word.includes(target))) inAllWord = false })
  return inAllWord;
};
...
const unenrollBrook = (arr) => {
  let result =  arr.map(students =>{
  let arrReturn = students.filter(name  =>{
    if (!(name.includes('Brook'))) { return name }
  })
  return arrReturn;
})
return result;
