...
const countNumberOfElements = (arr) => {
  let arrReturn = arr.reduce((acc ,value, idx) =>{ return idx+1; },0);
  return arrReturn;
};
...
const returnNames = (arr) => {
  let arrReturn = arr.reduce((acc,character, idx) =>{
...
const reversedString = (str) => {
  ...
  let arrReturn = arrStr.reduce((actualVal, letter, idx) =>{
    return actualVal = letter + actualVal;
  },'');
  return arrReturn;
};
...
const countNumberOfChildren = (arr) => {
  let numChildren = arr.reduce((accValue, family) =>{
    if ( family.children !== undefined){
      accValue += family.children.length;
    }
    return accValue;
  },0);
  return numChildren;
...
const calculateAverage = (arr) => {
  let numElem=0;
  let avg = arr.reduce((result, value) =>{
    numElem++;
    return ((result + value));
  },0);
  return (avg / numElem);
...
const countPrimeNumbers = (arr) => {
  let countPrimes = arr.reduce((acc,value) =>{
    if (isPrime(value)) {
      acc++
      return acc;
   }
